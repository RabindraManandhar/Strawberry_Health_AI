...
def capture_and_track(model_path, camera_index, save_frames_dir):
    ...
    # Ensure the directory to save frame exists
    ...
    while True:
        ...
        cv2.imshow("Camera" + str(camera_index), gray_frame)

        # Save the current frame as an image file
        frame_filename = f"Frame_{frame_count + 1}.jpg"
        frame_filepath = os.path.join(save_frames_dir, frame_filename)
        cv2.imwrite(frame_filepath, gray_frame) # Save the frame as an image
...
def save_classification_results(results_list, output_file_path):
    ...
    with open(output_file_path, 'w') as file:
        # Write the header
        file.write(f"{'Image':<20} {'Classification':<20} Detected Classes\n")
        file.write('-' * 70 + '\n')

        # Write each result to the file
        for result in results_list:
            detected_classes_str = ', '.join(result['detected_classes']) if result['detected_classes'] else 'None'
            file.write(f"{result['image']:<20} {result['classification']:<20} {detected_classes_str}\n")
